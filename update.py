...
mindsection_df = pd.read_csv("qhp_midsection (3) (1).csv")
hp_a2ch_df = pd.read_csv("qhp_a2ch (3) (1).csv")
qhp_costshare_df = pd.read_csv("qhp_costshare (2) (1).csv")

# Template sheet name for copying the structure
template_sheet_name = 'Benefits Package 1'

# Merge dataframes and create a new column 'hios_id'
merged_df = pd.merge(
    hp_a2ch_df,
    qhp_costshare_df,
...
# Function to create and populate Cost Share Variances sheets
def create_and_populate_cost_share_variances_sheet(base_sheet_name, new_sheet_name, data):
    ...
    # Write data starting from row 4, column 1 without changing formatting
    start_row = 4
    font = Font(name='Arial', size=11)
...
for key, value in dfs.items():
    ...
    if key in key_mapping:
        ...
        # Create and populate corresponding Cost Share Variances sheet
        hios_ids = mindsection_df['HIOS Plan ID* (Standard Component)'].unique()
        for hios_id in hios_ids:
            if hios_id in merged_df['hios_id'].values:
                cost_share_variances_sheet_name = f"Cost Share Variances {sheet_name.split()[-1]}"
                cost_share_data = merged_df[merged_df['hios_id'] == hios_id]
                create_and_populate_cost_share_variances_sheet("Cost Share Variances 1", cost_share_variances_sheet_name, cost_share_data)
